...
def extract_sites_to_lives_content(json_str):
    """从 JSON 字符串中提取从 'sites':[' 到 'lives' 之间的内容。"""
...
if source_response.status_code == 200 and target_response.status_code == 200:
    # Step 2: 提取源 JSON 中 'sites' 到 'lives' 部分的内容
    source_content = source_response.text
    target_content = target_response.text

    source_sites_to_lives_content = extract_sites_to_lives_content(source_content)
    
    if source_sites_to_lives_content:
        # Step 3: 替换目标 JSON 中的 'sites' 到 'lives' 部分
        target_content_updated = re.sub(
            r'("sites":\s*\[.*?\])(,?\s*"lives")',
            source_sites_to_lives_content,
            target_content,
            flags=re.DOTALL
        )
